...
public class HttpServerModule extends ReactContextBaseJavaModule implements LifecycleEventListener {
    ...
    @ReactMethod
    public void start(int port, String serviceName, ReadableMap opts) {
    ...
    @ReactMethod
    public void respondFile(String requestId, int code, String fileName, ReadableMap opts) {
        if (server != null) {
            server.respondFile(reactContext, requestId, code, fileName, opts);
        }
    }
    ...
    private void startServer(ReadableMap opts) {
        if (this.port == 0) {
            return;
        }

        if (server == null) {
            server = new Server(reactContext, port, opts);
        }
        try {
            server.start();
        } catch (IOException e) {
